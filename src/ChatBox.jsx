...
const ChatBox = ()=>{
    ...
    let [messages, setMessages] = useState([{'msg': 'hello', 'time': 'default'}, {'msg': 'hi', 'time': 'default'}])
    ...
    useEffect(()=>{
        ...
        .catch(()=>{
            console.log('Some Errorrr in ChatBox')
        })

    },[name])

    let msgData = (a)=>{
        setMsg(a.target.value)
    }
    ...
    let sendit = (e)=>{
        ...
        messages.push({'msg': msg, 'time': `${hours}:${minutes} ${hours>12? 'pm':'am'}`})
        setMsg('')
   ...
   if(user === undefined){
    ...
    return(
        <div id={style.ChatBox}>
            ...
            <form>
                <input type="text" placeholder='Type a message' value={msg} onChange={msgData} />
